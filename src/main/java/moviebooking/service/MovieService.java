...
public class MovieService {
    ...
    public Movie createMovie(Movie movie) {
        ...
        // VALIDATION: Required fields with length limits (DoS prevention)
        if (movie.getTitle() == null || movie.getTitle().trim().isEmpty()) {
            throw new IllegalArgumentException("Movie title cannot be empty");
        }
        if (movie.getTitle().length() > 255) {
            throw new IllegalArgumentException("Movie title too long (max 255 characters, got: " + movie.getTitle().length() + ")");
        }
        ...
        if (movie.getGenre().length() > 100) {
            throw new IllegalArgumentException("Movie genre too long (max 100 characters, got: " + movie.getGenre().length() + ")");
        }

        // VALIDATION: Description length limit (prevent DoS with GB-sized strings)
        if (movie.getDescription() != null && movie.getDescription().length() > 5000) {
            throw new IllegalArgumentException("Movie description too long (max 5000 characters, got: " + movie.getDescription().length() + ")");
        }

        // VALIDATION: Positive values with upper bounds
        ...
        if (movie.getPrice() > 10000.0) {
            throw new IllegalArgumentException("Movie price too high (max $10,000, got: $" + movie.getPrice() + ")");
        }
        ...
        if (movie.getDurationMinutes() > 1000) {
            throw new IllegalArgumentException("Movie duration too long (max 1000 minutes, got: " + movie.getDurationMinutes() + " minutes)");
        }
    ...
    public Movie updateMovie(Movie movie) {
        ...
        // VALIDATION: Required fields with length limits (same as create)
        if (movie.getTitle() == null || movie.getTitle().trim().isEmpty()) {
            throw new IllegalArgumentException("Movie title cannot be empty");
        }
        if (movie.getTitle().length() > 255) {
